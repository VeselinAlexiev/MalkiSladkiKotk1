...
namespace FileCanvasDrawer
{
    public partial class DrawerForm : Form
    {
        ...
        private const int CANVAS_HEIGHT = 459;
        private const int ZOOM_LEVEL = 2;

        private Stack<DrawingCanvas> undoStack;
        ...
        private void ButtonSave_Click(object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                this.canvas.SaveCanvasToFile(Path.GetFullPath(dialog.FileName));
            }
        }
        ...
        private void ButtonLoad_Click(object sender, EventArgs e)
        {
            ...
            DrawingCanvas newCanvas = this.canvas.LoadCanvasFromFile(Path.GetFullPath(dialog.FileName));
        ...
        private void DisplayCanvas(DrawingCanvas canvas, PictureBox pictureBoxCanvas)
        {
            ...
            for (int row = 0; row < this.pictureBoxCanvas.Height; row++)
            {
                for (int col = 0; col < this.pictureBoxCanvas.Width; col++)
                {
